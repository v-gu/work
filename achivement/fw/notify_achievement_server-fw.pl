...
sub check_pending() {
	...
	if (-e $pendingLog) {
		...
		unlink($tempLog);
		$logDate = Util::get_epoch2date(time);
		$log->log( level => 'info', message => "$logDate Finished importing old entries. Starting normal scan.\n" );
		print "Finished importing old entries. Starting normal scan.\n";
	}
}
...
my $sock = new IO::Socket::INET (
	...
	Listen => 5000,
	Reuse => 1);
die "Could not create socket on $sockPort! $!\n" unless $sock;

my $sockI = 0;
my $lastLine = time;
while($sock) {
#	if($sockI++%100==0 || time-$lastLine>120) {
#		check_pending();
#	}
	$log->log( level => 'info', message => "w" ); # wait - this is the only step that it's ok if we wait a bit, other steps need to complete as fast as possible
	my $conn = $sock->accept();
	while(<$conn>) {
		$log->log( level => 'info', message => "r" ); # read
		read_line($_);
		$lastLine = time;
	}
}
